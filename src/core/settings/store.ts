...
import { palettePeek } from '@/core/theme/palette';
...
function applyToDocument(settings: Settings, cover: CoverPalette | null = null): void {
  ...
  const wear = (theme: string | null) => {
    if (theme) root.dataset.theme = theme;
    else delete root.dataset.theme;
  };

  const wanted =
    settings.theme && settings.theme !== DEFAULT_THEME && isThemeId(settings.theme)
      ? settings.theme
      : null;

  /**
   * Where the palette is going, fetched before it sets off.
   *
   * The five built-in palettes are read back off the document rather than
   * copied into TypeScript, so a palette added later needs no telling. But the
   * variables are registered and transitioned, and read at the moment the theme
   * changes they give the palette on its way *out* — so everything derived from
   * them was a theme behind, the hairline between the player and the drawer
   * most visibly of all.
   */
  const arriving =
    wanted === (root.dataset.theme ?? null)
      ? readPalette(root)
      : palettePeek(root, wanted, wear, () => readPalette(root));
