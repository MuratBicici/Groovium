...
- **The window can be glass, and CSS cannot frost it.** Settings → Appearance has an opacity for the shell and a choice of platform effect under it. `backdrop-filter` was the obvious way to blur what shows through and does nothing here: it samples the page behind an element, and behind this one is the desktop, which belongs to the compositor rather than to the webview — so it frosts perfectly in a browser and not at all in the app. An adjustable blur radius does not exist on Windows for a window like this one; the only API with one needs `CreateHostBackdropBrush`, which returns a black visual outside UWP. Hence a choice of fixed effect through `window-vibrancy`, in `src-tauri/src/vibrancy.rs`, with the colour over it left to CSS.
